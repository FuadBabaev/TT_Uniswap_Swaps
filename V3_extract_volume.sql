...
DECLARE pool STRING  DEFAULT '0x4e68ccd3e89f51c3074ca5072bbac773960dfa36'; -- WETH/USDT 0.3% pool
...
const start = s.length - 6; // 6 hex chars = 24 bits
const chunk = '0x' + s.slice(start);
let v = BigInt(chunk);
if (v >= (BigInt(1) << BigInt(23))) v = v - (BigInt(1) << BigInt(24)); // sign
return Number(v);
""";
...
SELECT
  ...
  -- signed deltas, sign edfines the direction
  CAST(HEX_TO_INT256(w_amount0) AS BIGNUMERIC)                  AS amount0_raw,
  CAST(HEX_TO_INT256(w_amount1) AS BIGNUMERIC)                  AS amount1_raw,

  CAST(HEX_TO_INT256(w_amount0) / 1e18 AS BIGNUMERIC)           AS amount0_WETH,
  CAST(HEX_TO_INT256(w_amount1) / 1e6  AS BIGNUMERIC)           AS amount1_USDT,
